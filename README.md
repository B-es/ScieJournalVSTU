...
├── open-access-policy.vue, jobs.vue, help.vue,                          # информационные страницы
│   terms-of-use.vue, privacy-policy.vue, terms-and-conditions.vue,      # правовые документы
│   guidelines/reviewers.vue, guidelines/editors.vue                     # руководящие принципы
...
Публичные страницы используют компонент `<MarkdownContent>` (`src/components/public/`) для рендеринга Markdown-текста из i18n (`src/i18n/locales/{ru,en}.json`). Текст на каждой странице редактируется напрямую в ключах `text`, `empty`, `title` соответствующих секций.
...
Шапка (`AppHeaderPublic.vue`) содержит основное меню навигации, подвал (`AppFooter.vue`) — ссылки на информационные страницы: «Политика открытого доступа», «Работа в журнале», «Требования к авторам/рецензентам/редакторам», «Условия использования», «Политика конфиденциальности», «Условия и положения», «Помощь». Все тексты информационных страниц хранятся в i18n.
...
Публичные страницы («О журнале», «Требования к авторам», а также информационные страницы футера) используют компонент `<MarkdownContent>` для отображения текста из i18n (`src/i18n/locales/{ru,en}.json`). Тексты хранятся ключами `text` (сам текст), `title` (заголовок страницы), `empty` (сообщение когда текст не опубликован).

Компоненты (`src/components/`):

| Компонент | Назначение |
|---|---|
| `AppFooter.vue` | Подвал сайта с ссылками на информационные страницы |
| `AppHeaderPublic.vue` | Шапка сайта с навигацией и переключателем языков |
| `MarkdownContent.vue` | Рендеринг Markdown-текста в HTML для публичных страниц |
| `ArticleCard.vue` | Карточка статьи (публичные списки) |
| `IssueCard.vue` | Карточка выпуска журнала |
| `CitationBlock.vue` | Блок экспорта библиографической ссылки |
| `Pagination.vue` | Постраничная навигация |
| `AppFormField.vue` | Поле формы (shared) |
| `AppModal.vue` | Модальное окно (shared) |
| `StatusBadge.vue` | Бейдж статуса статьи (shared) |

Локализация — `@nuxtjs/i18n`, `src/i18n/locales/{ru,en}.json`, переключение без потери контекста (`strategy: "no_prefix"`). Контент статей/выпусков хранится билингвально в БД (`title_ru`/`title_en` и т.д.), а не переводится на лету. Тексты информационныx страниц хранятся непосредственно в i18n.
...
| Гость (не авторизован) | — | Публичный сайт: главная, архив, поиск, страница статьи, «О журнале», «Требования к авторам», все информационные страницы футера (открытый доступ, политика конфиденциальности, условия использования, помощь и т.д.) |
