...
import { listPublicArticles, listPublicIssues, type PublicArticle, type PublicIssue } from "~/api/public";
...
async function load() {
  ...
  error.value = "";
  try {
    const [issues, articles] = await Promise.all([listPublicIssues(), listPublicArticles({ pageSize: 6 })]);
