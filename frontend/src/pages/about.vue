...
const about = computed(() => (settings.value ? (locale.value === "en" ? settings.value.aboutEn : settings.value.aboutRu) : ""));
const board = computed(() => [...(settings.value?.editorialBoard ?? [])].sort((a, b) => a.order - b.order));
...
<template>
  <div>
    ...
    <template v-else-if="settings">
      <p v-if="settings.issn" class="about-page__issn">ISSN: {{ settings.issn }}</p>
      <p class="about-page__text">{{ about }}</p>

      <section v-if="board.length" class="about-page__board">
        <h2>{{ t("aboutPage.editorialBoard") }}</h2>
        <ul class="about-page__board-list">
          <li v-for="(member, idx) in board" :key="idx" class="about-page__board-member">
            <strong>{{ memberName(member) }}</strong>
            <span v-if="memberRole(member)"> — {{ memberRole(member) }}</span>
            <div v-if="memberAffiliation(member)" class="about-page__board-affiliation">{{ memberAffiliation(member) }}</div>
          </li>
        </ul>
      </section>
    </template>
