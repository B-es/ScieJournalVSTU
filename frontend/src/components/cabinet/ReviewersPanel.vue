...
async function submitReassign() {
    ...
    reassignError.value = "";
    try {
        await reviewsApi.reassignReviewer(
            reassigningId.value as string,
            newReviewerId.value,
            newDeadline.value,
        );
        reassigningId.value = null;
...
<template>
    <div class="reviewers-panel">
        ...
        <table v-else class="reviewers-panel__table">
            ...
            <tbody>
                <tr v-for="item in items" :key="item.id">
                    <td>
                        {{ item.reviewerFullName }} ({{ item.reviewerEmail }})
                    </td>
                    <td>{{ item.deadline }}</td>
                    <td>
                        <span
                            :class="`reviewers-panel__status reviewers-panel__status--${item.invitationStatus}`"
                        >
                            {{
        ...
        <div v-if="reassigningId" class="reviewers-panel__reassign-form">
            ...
            <label
                for="reassign-reviewer"
                class="reviewers-panel__select-label"
                >{{ t("reviewersPanel.newReviewer") }}</label
            >
            <select
                id="reassign-reviewer"
                v-model="newReviewerId"
                class="reviewers-panel__select"
            >
                <option value="" disabled>
                    {{ t("reviewersPanel.newReviewerPlaceholder") }}
                </option>
                <option
            ...
            <AppFormField
                v-model="newDeadline"
                type="date"
                :label="t('topicCheck.deadline')"
                required
            />

            <p v-if="reassignError" class="reviewers-panel__error" role="alert">
                {{ reassignError }}
            </p>
            ...
            <div class="reviewers-panel__actions">
                ...
                >
                    {{
                        isSubmitting
                            ? t("common.loading")
                            : t("reviewersPanel.reassignSubmit")
                    }}
                </button>
            </div>
        </div>
