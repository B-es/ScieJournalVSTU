...
const acceptedReviews = computed(() =>
    props.reviews.filter((r) => r.invitationStatus === "accepted"),
);
const submittedCount = computed(
    () => acceptedReviews.value.filter((r) => r.submittedAt).length,
);
const ready = computed(
    () =>
        acceptedReviews.value.length > 0 &&
        submittedCount.value === acceptedReviews.value.length,
);
...
async function submitDecision() {
    ...
    try {
        ...
        formError.value =
            err instanceof ApiError ? err.message : t("decisionForm.error");
    } finally {
...
async function fetchReviewers() {
    try {
        const reviewerRes = await reviewsApi.listMyReviews(
            undefined,
            props.articleId,
        );
        reviewers.value = reviewerRes.items;
    } catch (err) {
        formError.value =
            err instanceof ApiError ? err.message : t("decisionForm.error");
    }
...
function getColor(recommendation: string) {
    switch (recommendation) {
        case "accept":
            return "var(--color-status-accepted)";
        case "reject":
            return "var(--color-status-rejected)";
        case "revise":
            return "var(--color-status-revision)";
        default:
...
<template>
    <div class="decision-form">
        ...
        <ul class="decision-form__reviews">
            <li v-for="review in reviews" :key="review.id">
                <template v-if="review.submittedAt">
                    ...
                    <strong :style="{ color: getColor(review.recommendation) }">
                        {{
                            t(
                                `decisionForm.recommendation.${review.recommendation}`,
                            )
                        }}
                    </strong>
                    <p v-if="review.commentsForAuthor">
        ...
        <p v-if="!ready" class="decision-form__waiting">
            {{
                t("decisionForm.waiting", {
                    submitted: submittedCount,
                    total: acceptedReviews.length,
                })
            }}
        </p>
        ...
        <template v-else>
            <div class="decision-form__field">
                <label for="decision">{{
                    t("decisionForm.decisionLabel")
                }}</label>
                <select
                    id="decision"
                    v-model="decision"
                    class="decision-form__select"
                >
                    <option value="" disabled>
                        {{ t("decisionForm.decisionPlaceholder") }}
                    </option>
                    <option value="accept">
                        {{ t("decisionForm.recommendation.accept") }}
                    </option>
                    <option value="revise">
                        {{ t("decisionForm.recommendation.revise") }}
                    </option>
                    <option value="reject">
                        {{ t("decisionForm.recommendation.reject") }}
                    </option>
                </select>
            </div>
            ...
            <AppFormField
                v-model="comment"
                type="textarea"
                :label="t('decisionForm.comment')"
                required
            />

            <p v-if="fieldError" class="decision-form__error" role="alert">
                {{ fieldError }}
            </p>
            <p
                v-if="formError"
                class="decision-form__error"
                role="alert"
                aria-live="assertive"
            >
                {{ formError }}
        ...
        <AppModal
            ...
            :title="t('decisionForm.confirmRejectTitle')"
        >
